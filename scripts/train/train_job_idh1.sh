...
save_exp="tcga_${label}/${backbone}/${split_dir}/${model}/${mag}_1"
echo "$save_exp"
...
python main.py --drop_out 0 --early_stopping --lr 2e-4 --k 10 --label_frac 1.0 --exp_code "$save_exp" --task "$task" --patch_size 256 --backbone "$backbone" --results_dir results --model_type "$model" \
    --log_data --in_dim "$in_dim" --split_dir splits/${split_dir} --csv_path dataset_csv/df_idh_label.csv --features_dir /N/project/histopath/tcga_${organ}_features/features_${backbone}/tcga_${organ}_${mag} \
...
python eval_miccai.py --k 10 --models_exp_code "tcga_${label}/${backbone}/${split_dir}/${model}/${mag}_1_s1" --save_exp_code tcga_idh/${backbone}/${split_dir}/${model}/${mag} --task tcga_2_class --model_type ${model} \
    --results_dir results --data_root_dir /N/slate/sinnani/MambaMIL/ --split test --features_dir /N/project/histopath/tcga_${organ}_features/features_${backbone}/tcga_${organ}_${mag}/ \
    --csv_path dataset_csv/df_idh_label.csv --splits_dir splits/${split_dir} --in_dim ${in_dim} --embed_dim ${in_dim}

python eval_miccai.py --k 10 --models_exp_code "tcga_${label}/${backbone}/${split_dir}/${model}/${mag}_1_s1" --save_exp_code ebrains_idh/${backbone}/${split_dir}/${model}/${mag} --task tcga_2_class --model_type ${model} \
...
python eval_miccai.py --k 10 --models_exp_code "tcga_${label}/${backbone}/${split_dir}/${model}/${mag}_1_s1" --save_exp_code penn_idh/${backbone}/${split_dir}/${model}/${mag} --task tcga_2_class --model_type ${model} \
