...
def compute_w_loader(output_path, loader, model, verbose=0):
    ...
    for count, data in enumerate(tqdm(loader)):
        with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.float16):    
            ...
            # Get the features from the model's output
            model_output = model(batch)
            print(model_output)
            # Depending on your model, you might need to access a specific attribute, e.g., 'last_hidden_state'
            features = model_output.pooler_output  # or model_output.pooler_output, etc.
            features = features.to(torch.float16)
            features = features.cpu().numpy().astype(np.float32)
...
def compute_w_loader(output_path, loader, model, verbose = 0):
	...
	for count, data in enumerate(tqdm(loader)):
		with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.float16):
			...
			features = embedding.to(torch.float16)
			features = features.cpu().numpy().astype(np.float32)
